...
describe "amoStorage module", ->
  ...
  describe "localStorage", ->
    ...
    beforeEach ->
      ...
      inject ["amoStorageManager", (manager) ->
        storage = manager.getLocalStorage prefix
      ]

    it "should set '#{revisionsKey}' key on localStorage", ->
      expect(localStorage.getItem(revisionsKey)).not.toBe null

    it "should set array as '#{revisionsKey}' and it has 1 value on localStorage", ->
      item = angular.fromJson(localStorage.getItem(revisionsKey))
      expect(item).toEqual [revision]

    it "should not set '#{keysKey}' key on localStorage", ->
      expect(localStorage.getItem(keysKey)).toBe null
    ...
    describe "set method", ->
      ...
      it "should set '#{expectedKey}' key on localStorage", ->
        storage.set key, value
        expect(localStorage.getItem(expectedKey)).not.toBe null

      it "should set the object that has 'value' key on localStorage", ->
        storage.set key, value
        item = angular.fromJson localStorage.getItem expectedKey
        expect(item.value).toBe value

      it "should set the object that has 'config' key on localStorage", ->
        storage.set key, value
        item = angular.fromJson localStorage.getItem expectedKey
        expect(item.config).toBeDefined()

      describe "the config on localStorage", ->
      ...
      it "should set '#{keysKey}' key on localStorage", (done) ->
        inject ["$rootScope", ($rootScope) ->
          storage.set key, value
          $rootScope.$apply()
          setTimeout((->
            expect(localStorage.getItem(keysKey)).not.toBe null
            done()
          ), 110)
        ]

      it "should set the object as '#{keysKey}' on localStorage", (done) ->
        inject ["$rootScope", ($rootScope) ->
          storage.set key, value
          $rootScope.$apply()
          setTimeout((->
            item = angular.fromJson(localStorage.getItem(keysKey))
    ...
    describe "del method", ->
      ...
      it "should remove '#{expectedKey}' key on localStorage", ->
        storage.set key, value
        storage.del key
        expect(localStorage.getItem(expectedKey)).toBe null
  ...
  describe "updating revision", ->
    ...
    beforeEach ->
      localStorage.setItem revisionsKey, angular.toJson revisions
      localStorage.setItem old.keysKey, angular.toJson Object.keys(old.keys)
      localStorage.setItem(k, v) for k, v of old.keys
      localStorage.setItem prev.keysKey, angular.toJson Object.keys(prev.keys)
      localStorage.setItem(k, v) for k, v of prev.keys
    ...
    it "should remove all keys of prev revisions", ->
      ...
      inject ["amoStorageManager", (manager) ->
        expect(angular.fromJson localStorage.getItem revisionsKey).toEqual [current.revision]
        expect(localStorage.getItem old.keysKey).toBe null
        expect(localStorage.getItem k).toBe(null) for k of old.keys
        expect(localStorage.getItem prev.keysKey).toBe null
        expect(localStorage.getItem k).toBe(null) for k of prev.keys
      ]
    ...
    it "should not remove prev revision when setting ruleForDeletingRevisions", ->
      ...
      inject ["amoStorageManager", (manager) ->
        expect(angular.fromJson localStorage.getItem revisionsKey).toEqual [current.revision, prev.revision]
        expect(localStorage.getItem old.keysKey).toBe null
        expect(localStorage.getItem k).toBe(null) for k of old.keys
        expect(localStorage.getItem prev.keysKey).not.toBe null
        expect(localStorage.getItem k).not.toBe(null) for k of prev.keys
      ]
